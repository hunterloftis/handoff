...
Camera.prototype.render = function(seconds, state) {
  ...
  var dZoom = 1 / (state.player.currentSpeed * 0.001 + 1) - this.zoom;
  ...
  robot.position.x = state.player.x - this.x;
  robot.position.y = state.player.y - this.y;
  robot.scale.x = 1 + Math.abs(state.player.leanX) * 0.1;
  robot.scale.y = 1 + Math.abs(state.player.leanY) * 0.2;
  robot.rotation = state.player.leanX * Math.PI * 0.07;
