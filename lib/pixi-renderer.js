...
PixiRenderer.prototype.init = function(done) {
  ...
  function onLoad() {
    ...
    this.robotSprite = PIXI.MovieClip.fromFrames(charFrames);
    this.robotSprite.baseScale = 2.25;
    this.robotSprite.scale.set(this.robotSprite.baseScale);
...
PixiRenderer.prototype.render = function(seconds, state) {
  ...
  var leanX = robot.baseScale + Math.abs(player.ax * 0.01 + player.vx * 0.0001);
  var leanY = robot.baseScale - (player.ay * 0.011 + player.vy * 0.00008);
  var leanX2 = (player.ax * 0.006 + player.vx * 0.0001) * Math.PI;

  var playerAngle = (Math.atan2(player.vy, player.vx) + Math.PI * 2) % (Math.PI * 2);

  var playerDir
  if (between(12.5, 37.5)) playerDir = 8;
  else if (between(37.5, 62.5)) playerDir = 7;
  else if (between(62.5, 87.5)) playerDir = 6;
  else if (between(87.5, 112.5)) playerDir = 5;
  else if (between(112.5, 137.5)) playerDir = 1;
  else if (between(137.5, 162.5)) playerDir = 2;
  else if (between(162.5, 187.5)) playerDir = 3;
  else playerDir = 4;

  var playerFrame = playerSpeed > 10
    ? 8 - Math.floor(state.player.distance / 25) % 9
    : 8;
  robot.gotoAndStop(playerFrame + 9 * (playerDir - 1));

  function between(a, b) {
    return playerAngle >= Math.PI * (a / 100) && playerAngle <= Math.PI * (b / 100);
  }
