...
PixiPlayer.prototype.render = function(seconds, state) {
  ...
  var leanX = (state.ax * 0.006 + state.vx * 0.0001) * Math.PI * ss;
  ...
  var index = this.mc.parent.getChildIndex(this.mc);
  var stepIndex = state.vy > 0 ? index : index + 1;
  if (stepForce > 10 || (stepForce > 1 && pose % 4 === 0)) {
    PubSub.publish('/player/step', state.x, state.y, stepForce, stepIndex);
  }
};
