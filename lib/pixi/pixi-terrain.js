function PixiTerrain(ground, actors) {
  ...
  Object.keys(PIXI.TextureCache).forEach(function(textureName) {
    ...
    if (textureDetails) {
      ...
      // B0 = Blob
      // B1 = Worn stone with etched crop-circle-like designs
      // D0 = Gravelly dirt with etched markings
      // D1 = Dirt
      // D2 = Red dirt
      // D3 = Dirt with patches of grass
      // G0 = deep green grass
      // G1 = lighter grass
      // G2 = mossy grass over large stones
      // G3 = moss on small cobblestones
      // P0 = green foresty leaves
      // P1 = brilliant fall leaves
      // R0 = Ramp
      // S0-S2 = Stone
  ...
  console.log('using texture group:', baseEnvTexture);
  ...
  PubSub.subscribe('pixi/player/step', this.onStep.bind(this));
}
...
PixiTerrain.prototype.createDustTexture = function() {
  ...
  gfx.beginFill(0xe2c99C);
  for (var i = 0; i < 10; i++) {
    gfx.drawCircle(Math.random() * 10, Math.random() * 10, 1);
  }
  gfx.endFill();
...
PixiTerrain.prototype.render = function(seconds, state, rect) {
  ...
  function renderDust(sprite) {
    ...
    sprite.scale.set(sprite.scale.x + seconds * 2);
